...
#define COMPARE(key, nodekey, compare)                                         \
  ((compare == avl_numcmp) ? (intptr_t)key - (intptr_t)nodekey :               \
                             (*compare)(key, nodekey))
...
delete_item:
  *key_p = node->key;
  ...
  if(node->left == NIL(avl_node)) { *node_p = node->right; }
  else {
...
avl_generator *avl_init_gen(avl_tree *tree, int dir)
{
  ...
  gen->count = 0;
  if(dir == AVL_FORWARD) { avl_record_gen_forward(tree->root, gen); }
  else {
...
int avl_gen(avl_generator *gen, void **key_p, void **value_p)
{
  ...
  if(gen->count == gen->tree->num_entries) { return 0; }
  else {
...
static void do_rebalance(avl_node ***stack_nodep, int stack_n)
{
  ...
  while(--stack_n >= 0) {
    ...
    if((hr - hl) < -1) { rotate_right(node_p); }
    else if((hr - hl) > 1) {
      rotate_left(node_p);
    }
    else {
                         ...
                         int *error)
{
  ...
  if(node->left != NIL(avl_node) && (*compar)(node->left->key, node->key) > 0) {
    (void)printf("Bad ordering between %p and %p", (void *)node,
                 (void *)node->left);
    ++*error;
  }
